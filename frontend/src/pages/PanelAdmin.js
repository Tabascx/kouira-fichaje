...
export default function PanelAdmin() {
  ...
  // Cargar trabajadores SIEMPRE al montar — necesario para fichaje manual y ausencias
      ...
      <div className="panel-wrap">
          ...
          <div className="seccion">
            <div className="seccion-header">
              <div className="seccion-titulo" style={{ margin: 0 }}>
                {t('fichajes_hoy')} — {new Date().toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}
              </div>
              ...
              <div className="tabla-wrap">
                {fichajesHoy.map((f) => (
                  <div key={f.id} className="fila-fichaje">
                    ...
                    <button className="btn-mini rojo" onClick={() => eliminarFichaje(f.id)} title="Eliminar">✕</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === 'ausencias' && (
          <div className="seccion">
            ...
            {trabajadoresSolo.length === 0 ? (
              <div className="vacio" style={{ fontSize: 12 }}>Cargando trabajadores...</div>
            ) : (
              <div className="chips-trabajadores">
                {trabajadoresSolo.map(trab => (
                  <button key={trab.id} className="btn-trabajador"
                    onClick={() => {
                      setModalAusencia({ usuario_id: trab.id, nombre: trab.nombre });
                      setFormAusencia({ fecha: new Date().toISOString().slice(0,10), justificada: false, motivo_tipo: '', motivo: '' });
                    }}>
                    + {trab.nombre.split(' ')[0]}
                  </button>
                ))}
        ...
        {modalFichajeManual && (
          <div className="modal-fondo" onClick={() => setModalFichajeManual(false)}>
            <div className="modal-card" onClick={(e) => e.stopPropagation()}>
              <div className="modal-titulo">{t('fichaje_manual')}</div>
              <form onSubmit={crearFichajeManual} className="form-nuevo">
                <label className="form-label">{t('usuario_del_fichaje')}</label>
                <select
                  value={formFichajeManual.usuario_id}
                  onChange={(e) => setFormFichajeManual({ ...formFichajeManual, usuario_id: e.target.value })}
                  required
                ...
                <select
                  value={formFichajeManual.tipo}
                  onChange={(e) => setFormFichajeManual({ ...formFichajeManual, tipo: e.target.value })}
                  className="form-select"
                >
                  <option value="entrada">{t('entrada')}</option>
                  <option value="salida">{t('salida')}</option>
                </select>
                <label className="form-label">{t('hora_fichaje')}</label>
                <input
                  type="datetime-local"
                  value={formFichajeManual.fecha_hora}
                  onChange={(e) => setFormFichajeManual({ ...formFichajeManual, fecha_hora: e.target.value })}
                  required
                  className="form-input"
                />
                <div className="modal-btns">
                  <button type="button" className="btn-logout" onClick={() => setModalFichajeManual(false)}>{t('cancelar')}</button>
                  <button type="submit" className="btn-login">{t('crear_fichaje_manual')}</button>
                </div>
              </form>
            </div>
          </div>
